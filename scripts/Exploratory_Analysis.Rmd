---
title: "Exploratory Plots to Characterise the Dataset"
author: "Caitlin M"
date: "`r Sys.Date()`"
output: 
  html_document:
    toc: True
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = FALSE)
#knitr::opts_knit$set(root.dir = "/Volumes/phd") # if running in datastore
knitr::opts_knit$set(root.dir = "/exports/cmvm/eddie/sbms/groups/young-lab/caitlin/phd") # if running in Eddie
knitr::opts_chunk$set(message = FALSE)
knitr::opts_chunk$set(warning = FALSE)

library(ggplot2)
library(tidyverse)
library(data.table)
library(VennDiagram)
library(UpSetR)
library(grid)
library(scales)
library(patchwork)
library(ggpattern)

```

# This code is an exploration between the relationship between the variants found in the different populations as well as the overlap between various phenotype groups (as defined by the FANTOM consortium).
#### This exploratory analysis will reveal underlying patterns in the data that may not be immediately evident and provide a better understanding of the data.


```{r files}
# Read in population files and give them column names
AFR <- fread("1KGP_hg38/AFR_variants_sorted.vcf", header = F)
AMR <- fread("1KGP_hg38/AMR_variants_sorted.vcf", header = F)
EAS <- fread("1KGP_hg38/EAS_variants_sorted.vcf", header = F)
EUR <- fread("1KGP_hg38/EUR_variants_sorted.vcf", header = F)
SAS <- fread("1KGP_hg38/SAS_variants_sorted.vcf", header = F)

# Assign consistent column names
colnames(AFR) <- colnames(AMR) <- colnames(EAS) <- colnames(EUR) <- colnames(SAS) <- 
  c("CHR", "start_pos", "end_pos", "strand", "REF", "ALT", "AF") 

# Create unique SNP IDs (e.g., "chr1:12345")
variant_lists <- list(
  AFR = paste0("chr", AFR$CHR, ":", AFR$start_pos),
  AMR = paste0("chr", AMR$CHR, ":", AMR$start_pos),
  EAS = paste0("chr", EAS$CHR, ":", EAS$start_pos),
  EUR = paste0("chr", EUR$CHR, ":", EUR$start_pos),
  SAS = paste0("chr", SAS$CHR, ":", SAS$start_pos)
)
```
## Number of Variants per Population
#### These plots look at the the number of variants per population. There is one plot that is the total number of variants in the population and the second plot is the number of unique variants - variants that only appear in one population - for each superpopulation

```{r pop}

# read in files for the total number of variants for each superpopulation
pop <- read.table("1KGP_hg38/pop_count_hg38.txt", sep = "\t")
colnames(pop) <- c("Population", "Total_Lines")
bootstrapped <- read.table("preliminary_exploration/stats/bootstrap_results.txt", sep = "\t", header = T) # bootstrapping results for the total variant count
colnames(bootstrapped)[1] <- "Population"
pop_boot <- full_join(pop, bootstrapped, by = "Population")
# adjusting the lines so they are not in scientific notation
pop_boot$Adjusted_lines <- pop_boot$Total_Lines / 1e7
pop_boot$Adjusted_uCI <- pop_boot$Upper_CI / 1e7
pop_boot$Adjusted_lCI <- pop_boot$Lower_CI / 1e7


# read in files for the  number of variants unique to each superpopulation
unique_pop <- read.table("1KGP_hg38/unique_pop_count_hg38.txt", sep = "\t")
colnames(unique_pop) <- c("Population", "Lines")
unique_bootstrap <- read.table("preliminary_exploration/stats/unique_bootstrap_results.txt", sep = "\t", header = T)
colnames(unique_bootstrap)[1] <- "Population"
uniq_pop_boot <- full_join(unique_pop, unique_bootstrap, by = "Population")
# adjusting the lines so they are not in scientific notation
uniq_pop_boot$Adjusted_lines <- uniq_pop_boot$Lines / 1e6
uniq_pop_boot$Adjusted_uCI <- uniq_pop_boot$Upper_CI / 1e6
uniq_pop_boot$Adjusted_lCI <- uniq_pop_boot$Lower_CI / 1e6

# plotting total variants per population
pop_plot <- ggplot(pop_boot, aes(Population, Adjusted_lines, fill = Population)) + geom_bar(stat="identity") +
  geom_errorbar(aes(x = Population, ymin = Adjusted_lCI, ymax = Adjusted_uCI)) +
  ylab(bquote("Number of Variants "(x10^7))) +
  xlab("Population") +
  theme_classic() +
  theme(legend.position="none", plot.title = element_text(hjust = 0.5)) +
  ggtitle("Number of Variants in 1000 Genomes Project Populations") 
pop_plot
# Save high-resolution image
#ggsave("figures/population_variant_count.png", pop_plot, dpi = 300, width = 6, height = 4)

# plotting the number of unique variant per population
unique_pop_plot <- ggplot(uniq_pop_boot, aes(Population, Adjusted_lines, fill = Population)) + geom_bar(stat="identity") +
  geom_errorbar(aes(x = Population, ymin = Adjusted_lCI, ymax = Adjusted_uCI)) +
  ylab(bquote("Number of Unique Variants "(x10^6))) +
  xlab("Population") +
  theme_classic() +
  theme(legend.position="none", plot.title = element_text(hjust = 0.5))+
  ggtitle("Number of Population-Specific Variants in 1000 Genomes Project Populations")
unique_pop_plot
#ggsave("unique_population_variant_count.png", unique_pop_plot, dpi = 300, width = 6, height = 4)


```

## Variant per chromosome count
#### There are two plots that are looking at the number of variants per chromosome for each of the superpopulations. The first plot looks at the number of variants per chromosome, whereas the second plot looks at the the number of variants in the chromosome adjusted by the size of the chromosome.

```{r chrom_count, fig.width=16, fig.height=10}
# read in file with the number of positions in each chromosome
hg38 <-  read.table("hg38.genome", header = T)

# Function to process and normalise counts per population
get_normalised_counts <- function(df, name) {
  df %>%
    filter(CHR %in% hg38$CHR) %>%
    count(CHR, name = "Variant_Count") %>%
    left_join(hg38, by = "CHR") %>%
    mutate(Population = name,
           Variants_per_bp = Variant_Count / size)
}

# Apply to each population
afr_norm <- get_normalised_counts(AFR, "AFR")
amr_norm <- get_normalised_counts(AMR, "AMR")
eas_norm <- get_normalised_counts(EAS, "EAS")
eur_norm <- get_normalised_counts(EUR, "EUR")
sas_norm <- get_normalised_counts(SAS, "SAS")

# Combine all
all_norm <- bind_rows(afr_norm, amr_norm, eas_norm, eur_norm, sas_norm)

# Clean up chromosome factor levels
all_norm$CHR <- factor(all_norm$CHR, levels = c(1:22, "X"))

# Histogram of the number of variants per population 
ggplot(all_norm, aes(x = CHR, y = Variant_Count, fill = Population)) +
  geom_bar(stat = "identity") +
  facet_wrap(~Population, scales = "free_y") +
  ylab("Number of Variants") +
  xlab("Chromosome") +
  ggtitle("Number of Variants in the 1000 Genomes Project Per Chromosome") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), , legend.position = "none", plot.title = element_text(hjust = 0.5))

# Histogram of the normalised counts per chromosome for each population
chr_plt <- ggplot(all_norm, aes(x = CHR, y = Variants_per_bp, fill = Population)) +
  geom_bar(stat = "identity") +
  facet_wrap(~Population, scales = "free_y") +
  ylab("Normalized Variant Density (variants per base pair)") +
  xlab("Chromosome") +
  ggtitle("Number of Variants in the 1000 Genomes Project Per Chromosome, Adjusted by the Size of the Chromosome") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "none", plot.title = element_text(hjust = 0.5))
chr_plt
#ggsave("figures/normalised_variants_per_chromosome.png", chr_plt, width = 12, height = 7)
```

```{r presence-matrix}
## Create Presence Matrix and Summary Table

# All unique SNPs across all populations
all_snps <- unique(unlist(variant_lists))

# Build binary matrix (presence/absence)
presence_matrix <- sapply(variant_lists, function(snps) all_snps %in% snps)
rownames(presence_matrix) <- all_snps

# Count in how many populations each SNP appears
population_count <- rowSums(presence_matrix)

# Create summary table
summary_table <- as.data.frame(table(population_count))
colnames(summary_table) <- c("Num_Populations", "Num_Variants")

# Save summary table
#write.table(summary_table, "snp_overlap_summary.txt", sep = "\t", row.names = FALSE, quote = FALSE)
```

## UpSet Plot for All Populations
#### This plot shows the relationship between the superpopulations by counting the number of variants that appear in the different ralationship and measuring the overlap between them. From this we can see which populations have the highest degree of relatedness

```{r upset-plot, fig.width=16, fig.height=10}
# Convert binary matrix to data frame for UpSetR
# Ensure binary (0/1) data frame
presence_df <- as.data.frame(presence_matrix)
presence_df <- presence_df %>%
  mutate(across(everything(), ~ as.integer(.)))

set_sizes <- colSums(presence_df)

# Sort population names by size (smallest to largest)
sorted_sets <- names(sort(set_sizes, decreasing = T))

set_colors <- c(
  AFR = "#F8766D",
  AMR = "#A3A500",
  EAS = "#00BF7D",
  EUR = "#00B0F6",
  SAS = "#E76BF3"
)

#png("figures/upset_plot.png", width = 1200, height = 800, res = 150)
UpSetR::upset(
  presence_df,
  nsets = length(sorted_sets),
  sets = sorted_sets,
  order.by = "freq",
  sets.bar.color = set_colors[sorted_sets],
  main.bar.color = "#333333",         
  matrix.color = "black",              # Matrix dots remain black
  matrix.dot.alpha = 1,
  set_size.show = F,
  text.scale = c(1.7, 1.6, 1.4, 0, 1.7, 0),
  keep.order = TRUE
)
#dev.off()
```

## Venn Diagrams of the Overlap with the European superpopulation
#### These venn diagrams look at the overlap between the European superpopulation and the other 4. 
#### We are looking at the European overlaps due to the extreme European bias in genome-wide association studies.

```{r venn-diagram, fig.width=6, fig.height=6}
# Create a function that generates a venn diagram between two superpopulations and asks for the colour to plot them with
plot_venn <- function(pop1, pop2, col1, col2, title){
  venn_obj <- venn.diagram(
    x = variant_lists[c(pop1, pop2)],
    category.names = c(pop1, pop2),  # pop1 appears on the left
    filename = NULL,
    output = TRUE,
    main = title,
    imagetype = "png",
    col = "transparent",
    fill = c(col1, col2),
    alpha = 0.5,
    cex = 1.2,
    cat.cex = 1.2,
    print.mode = "raw",
    percent = FALSE
  )
  
  grid.newpage()
  grid.draw(venn_obj)
}

```

```{r ven_eur_afr, fig.width=5, fig.height=5}
####Venn Diagram of EUR and AFR
plot_venn("EUR", "AFR", "#00B0F6", "#F8766D", "Venn Diagram of the Overlap Between EUR and AFR Population Variants")
```

```{r ven_eur_amr, fig.width=5, fig.height=5}
#Venn Diagram of EUR and AMR
grid.newpage()
grid.draw(plot_venn("EUR", "AMR", "#00B0F6", "#A3A500", "Venn Diagram of the Overlap Between EUR and AMR Population Variants"))
```
```{r ven_eur_eas, fig.width=5, fig.height=5}
#Venn Diagram of EUR and EAS

#png("figures/EUR_EAS_venn_diagram.png", width = 800, height = 800, res = 150)
grid.newpage()
grid.draw(plot_venn("EUR", "EAS", "#00B0F6", "#00BF7D", "Venn Diagram of the Overlap Between EUR and EAS Population Variants"))
#dev.off()
```

```{r ven_eur_sas, fig.width=5, fig.height=5}
#Venn Diagram of EUR and SAS
grid.newpage()
grid.draw(plot_venn("EUR", "SAS", "#00B0F6", "#E76BF3", "Venn Diagram of the Overlap Between EUR and SAS Population Variants"))
```

### Bar Plot of the number of populations that a variant is shared between

```{r snp-sharing, fig.width=6, fig.height=4}
ggplot(summary_table, aes(x = as.factor(Num_Populations), y = (Num_Variants/1e6))) +
  geom_bar(stat = "identity", fill = "steelblue") +
  xlab("Number of Populations") +
  ylab(bquote("Number of Variants "(x10^7))) +
  ggtitle("A Bar Plot of How Many Populations a Variant is Present in") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme_classic()
#ggsave("figures/shared_variants.png", dpi = 300, width = 6, height = 4)
```

## Allele Frequency
### Histogram of the allele frequencies across different populations

```{r AF, echo=FALSE}
pops <- c('AFR', 'AMR', 'EAS', 'EUR', 'SAS')
af_list <- list()
af1 <- data.frame(Population = character(), AF_1_count = integer(), stringsAsFactors = FALSE)

for (pop in pops){
  af_data <- get(pop) %>% count(AF) %>% as.data.frame()
  assign(paste0("af_", pop), af_data)
  
  #af_list[[pop]] <- af_data
  af1_count <- af_data[af_data$AF == 1.00, "n"]
  af1 <- rbind(af1, data.frame(Population = pop, AF_1_count = af1_count))
}

afr_plot <- ggplot(af_AFR, aes(AF, (n/1e6))) + geom_bar(stat = "identity", fill = "#F8766D") +
  xlab("Allele Frequency") +
  ylab(bquote("Number of Variants "(x10^6))) +
  ggtitle("The Distribution of Allele Frequencies in AFR") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme_classic()
afr_plot

amr_plot <- ggplot(af_AMR, aes(AF, (n/1e6))) + geom_bar(stat = "identity", fill = "#A3A500") +
  xlab("Allele Frequency") +
  ylab(bquote("Number of Variants "(x10^6))) +
  ggtitle("The Distribution of Allele Frequencies in AMR") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme_classic()
amr_plot

eas_plot <- ggplot(af_EAS, aes(AF, (n/1e6))) + geom_bar(stat = "identity", fill = "#00BF7D") +
  xlab("Allele Frequency") +
  ylab(bquote("Number of Variants "(x10^6))) +
  ggtitle("The Distribution of Allele Frequencies in EAS") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme_classic()
eas_plot

eur_plot <- ggplot(af_EUR, aes(AF, (n/1e6))) + geom_bar(stat = "identity", fill = "#00B0F6") +
  xlab("Allele Frequency") +
  ylab(bquote("Number of Variants "(x10^6))) +
  ggtitle("The Distribution of Allele Frequencies in EUR") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme_classic()
eur_plot

sas_plot <- ggplot(af_SAS, aes(AF, (n/1e6))) + geom_bar(stat = "identity", fill = "#E76BF3") +
  xlab("Allele Frequency") +
  ylab(bquote("Number of Variants "(x10^6))) +
  ggtitle("The Distribution of Allele Frequencies in SAS") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme_classic()
sas_plot
```

### Violin plots of the different allele frequencies within each chromosome

```{r violin_AF}
# Fixing the order of the Chromosomes
AFR$CHR <- factor(AFR$CHR, levels = c(1:22, "X"))
AMR$CHR <- factor(AMR$CHR, levels = c(1:22, "X"))
EAS$CHR <- factor(EAS$CHR, levels = c(1:22, "X"))
EUR$CHR <- factor(EUR$CHR, levels = c(1:22, "X"))
SAS$CHR <- factor(SAS$CHR, levels = c(1:22, "X"))

# AFR 
ggplot(AFR, aes(x = CHR, y = AF)) +
  geom_violin(trim = FALSE, alpha = 0.7, fill = "#F8766D") +
  labs(title = "Allele Frequency Distribution per Chromosome (AMR)",
       x = "Chromosome", y = "Allele Frequency") +
  theme_minimal() +
  theme(legend.position = "none", plot.title = element_text(hjust = 0.5))

# AMR 
ggplot(AMR, aes(x = CHR, y = AF)) +
  geom_violin(trim = FALSE, alpha = 0.7, fill = "#A3A500") +
  labs(title = "Allele Frequency Distribution per Chromosome (AMR)",
       x = "Chromosome", y = "Allele Frequency") +
  theme_minimal() +
  theme(legend.position = "none", plot.title = element_text(hjust = 0.5))

# EAS 
ggplot(EAS, aes(x = CHR, y = AF)) +
  geom_violin(trim = FALSE, alpha = 0.7, fill = "#00BF7D") +
  labs(title = "Allele Frequency Distribution per Chromosome (EAS)",
       x = "Chromosome", y = "Allele Frequency") +
  theme_minimal() +
  theme(legend.position = "none", plot.title = element_text(hjust = 0.5))

ggplot(EUR, aes(x = CHR, y = AF)) +
  geom_violin(trim = FALSE, alpha = 0.7, fill = "#00B0F6") +
  labs(title = "Allele Frequency Distribution per Chromosome (EUR)",
       x = "Chromosome", y = "Allele Frequency") +
  theme_minimal() +
  theme(legend.position = "none", plot.title = element_text(hjust = 0.5))

# SAS 
ggplot(SAS, aes(x = CHR, y = AF)) +
  geom_violin(trim = FALSE, alpha = 0.7, fill = "#E76BF3") +
  labs(title = "Allele Frequency Distribution per Chromosome (SAS)",
       x = "Chromosome", y = "Allele Frequency") +
  theme_minimal() +
  theme(legend.position = "none", plot.title = element_text(hjust = 0.5))
```

### Cumulative Distribution plots of the Allele Frequencies of each population

```{r cumulative_af}
AFR_af <- AFR  %>% select(AF) %>% mutate(Population = "AFR") 
AMR_af <- AMR  %>% select(AF)%>% mutate(Population = "AMR")
EAS_af <- EAS  %>% select(AF)%>% mutate(Population = "EAS")
EUR_af <- EUR  %>% select(AF) %>% mutate(Population = "EUR")
SAS_af <- SAS  %>% select(AF)%>% mutate(Population = "SAS")

# Combine into one long dataframe
all_af <- bind_rows(AFR_af, AMR_af, EAS_af, EUR_af, SAS_af)


ggplot(all_af, aes(x = AF, color = Population)) +
  stat_ecdf(size = 1) +
  theme_classic() +
  labs(title = "Cumulative Distribution of Allele Frequencies by Superpopulation",
       x = "Allele Frequency",
       y = "Cumulative Proportion")+
  theme(plot.title = element_text(hjust = 0.5))

superpopulations <- list(AFR = AFR_af, AMR = AMR_af, EAS = EAS_af, EUR = EUR_af, SAS = SAS_af)

cdf_data <- do.call(rbind, lapply(names(superpopulations), function(pop) {
  df <- superpopulations[[pop]] %>% arrange(AF)
  df$CDF <- seq_along(df$AF) / nrow(df)
  df$Population <- pop
  df
}))

bin_width <- 0.01
smoothed_cdf_binned <- cdf_data %>%
  mutate(AF_bin = cut(AF, breaks = seq(0, 1, by = bin_width), include.lowest = TRUE)) %>%
  group_by(Population, AF_bin) %>%
  summarise(
    AF_mid = mean(AF, na.rm = TRUE),
    CDF = max(CDF),
    .groups = "drop"
  )

# Plot
ggplot(smoothed_cdf_binned, aes(x = AF_mid, y = CDF, color = Population)) +
  geom_line(size = 1) +
  theme_classic() +
  labs(title = "Smoothed Cumulative Distribution of Allele Frequencies by Superpopulation",
    x = "Allele Frequency",
    y = "Cumulative Proportion") +
  scale_x_continuous(expand = c(0,0), limits = c(0,1)) +
  theme(
    text = element_text(size = 14), plot.title = element_text(hjust = 0.5)
  )
#ggsave("figures/cumulative_AFs.png", dpi = 300, width = 8, height = 4)

```

### Fixed Alleles

```{r fixed_alleles, echo=FALSE}
# a for loop that gets the distribution of the allele frequencies for each population
pops <- c('AFR', 'AMR', 'EAS', 'EUR', 'SAS')
af_list <- list()
af1 <- data.frame(Population = character(), AF_1_count = integer(), stringsAsFactors = FALSE)

for (pop in pops){
  af_data <- get(pop) %>% count(AF) %>% as.data.frame()
  assign(paste0("af_", pop), af_data)
  
  #af_list[[pop]] <- af_data
  af1_count <- af_data[af_data$AF == 1.00, "n"]
  af1 <- rbind(af1, data.frame(Population = pop, AF_1_count = af1_count))
}

ggplot(af1, aes(Population, (AF_1_count/1e5), fill = Population)) + geom_bar(stat = "identity") + 
  theme_classic() +
  theme(legend.position = "none") +
  xlab("Superpopulation") +
  ylab(bquote("Number of Variants "(x10^5))) +
  ggtitle("Number of Fixed Alleles (AF=1) in Each Population") + 
  theme(plot.title = element_text(hjust = 0.5))
#ggsave("figures/fixed_variants.png", dpi=300, width = 6, height = 4)
  
  
```

## Population Phenotypes
#### These plots are generated from the bedtools summary of the overlap between superpopulation variants and phenotype groups. The second plot is adjusted for the number of variants in the superpopulation to account for the differences in the number of variants in different populations

```{r phenotype_dist, echo=FALSE}
# reading in the bedtools summary file and manipulating the rows so they can be plotted
lines <- read_lines("gwas_1000_genomes/bedtools_summary.txt")[-1] # reads in the lines of the file, excluding the first line
bedtools <- tibble(raw = lines) %>%
  mutate(raw = str_remove(raw, " lines$")) %>%  # Remove " lines" from the end
  separate(raw, into = c("Population", "Phenotype"), sep = "_vs_", extra = "merge") %>%  # Split at "_vs_"
  separate(Phenotype, into = c("Phenotype", "Line_Count"), sep = ": ") %>%  # Split at ": "
  mutate(Phenotype = str_remove(Phenotype, "_gwas$"),  # Remove "_gwas"
         Line_Count = as.integer(Line_Count))  # Convert count to integer
#bedtools <- read.table("gwas_1000_genomes/bedtools_summary.txt", sep = "\t", header = T)
colnames(bedtools) <- c("Population", "Phenotype", "Line_Count")
bedtools2 <- bedtools[bedtools$Phenotype !='all',]

#reread in population count file
pop <- read.table("1KGP_hg38/pop_count_hg38.txt", sep = "\t")
colnames(pop) <- c("Population", "Total_Lines")
  
bed_boot <- read.table("preliminary_exploration/stats/bootstrap_superpop_phenotype_results.txt", sep = "\t", header = T)
colnames(bed_boot)[1] <- "Population"
bedtools_boot <- full_join(bedtools2, bed_boot, by = c("Population", "Phenotype"))
bedtools_boot <- subset(bedtools_boot, select = -c(Line_Count))
# creating a new dataframe that merges the dataframes with the number of variants and the phenotypes so the proportion can be calculated
proportion_df <- full_join(pop, bedtools_boot, by ="Population")
proportion_df <- proportion_df[order(proportion_df$Population),]
proportion_df$Proportion <- proportion_df$Count / proportion_df$Total_Lines *100
proportion_df$Adjusted_uCI <- proportion_df$Upper_CI / proportion_df$Total_Lines *100
proportion_df$Adjusted_lCI <- proportion_df$Lower_CI / proportion_df$Total_Lines *100

phenotype_order <-  c('Cancer', 'Neurological', 'Immunological') # changing the order of the phenotypes so it is more aesthetic
# plotting the raw numbers of the overlap between the variants and the phenotype groups
bedtools_plot <- ggplot(bedtools_boot, aes(factor(Phenotype, level = phenotype_order), Count, fill = Population)) + 
  geom_bar(stat = "identity",position=position_dodge(), color="white",) + 
  geom_errorbar(aes(ymin = Lower_CI, ymax = Upper_CI), 
                position = position_dodge(width = 0.9), 
                width = 0.25, 
                color = "black") +
  xlab("Phenotype") +
  ylab("Number of Variants") +
  ggtitle("The Number of Variants in Each Subcategory of Phenotypes by Population") + 
  labs(fill = "Population") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme_classic()
bedtools_plot
#ggsave("figures/phenotype_variant_count.png", bedtools_plot, dpi = 300, width = 7, height = 5)

# plotting the proportion of the phenotype overlap compared to the total number of variants for the population
prop_plot <- ggplot(proportion_df, aes(factor(Phenotype, level = phenotype_order), Proportion, fill = Population)) + 
  geom_bar(stat = "identity",position=position_dodge(), color="white",) + 
  geom_errorbar(aes(ymin = Adjusted_lCI, ymax = Adjusted_uCI), 
                position = position_dodge(width = 0.9), 
                width = 0.25, 
                color = "black") +
  xlab("Phenotype") +
  ylab("Propotion of the total variants (%)") +
  ggtitle("The Proportion of Total Population-Variants in Each Subcategory of Phenotypes by Population") + 
  theme(plot.title = element_text(hjust = 0.5)) +
  theme_classic()
prop_plot
#ggsave("figures/phenotype_variant_proportion.png", prop_plot, dpi = 300, width = 7, height = 5)

```

## GWAS phenotype data

```{r pheno_chr, echo=FALSE}

# read in file with the number of positions in each chromosome
hg38 <-  read.table("hg38.genome", header = T)

# Function to process and normalise counts per population
normalise_counts <- function(df, name) {
  df %>% mutate(CHR = as.character(CHR)) %>% 
    filter(CHR %in% hg38$CHR) %>%
    count(CHR, name = "Variant_Count") %>%
    left_join(hg38, by = "CHR") %>%
    mutate(Population = name,
           Variants_per_bp = Variant_Count / size)
}

AFR_pheno <- read.table("gwas_1000_genomes/AFR_all_gwas.bed", header = F, sep = "\t")
AMR_pheno <- read.table("gwas_1000_genomes/AMR_all_gwas.bed", header = F, sep = "\t")
EAS_pheno <- read.table("gwas_1000_genomes/EAS_all_gwas.bed", header = F, sep = "\t")
EUR_pheno <- read.table("gwas_1000_genomes/EUR_all_gwas.bed", header = F, sep = "\t")
SAS_pheno <- read.table("gwas_1000_genomes/SAS_all_gwas.bed", header = F, sep = "\t")

colnames(AFR_pheno) <- colnames(AMR_pheno) <- colnames(EAS_pheno) <- colnames(EUR_pheno) <- colnames(SAS_pheno) <- c("CHR", "start_pos", "end_pos", "RSID", "Phenotype","risk_AF", "p_value", "OR", "PMID", "v_CHR","v_start_pos", "v_end_pos", "v_rsid", "REF", "ALT", "AF", "overlap")

# Apply to each population
afr_norm <- normalise_counts(AFR_pheno, "AFR")
amr_norm <- normalise_counts(AMR_pheno, "AMR")
eas_norm <- normalise_counts(EAS_pheno, "EAS")
eur_norm <- normalise_counts(EUR_pheno, "EUR")
sas_norm <- normalise_counts(SAS_pheno, "SAS")

# Combine all
all_norm <- bind_rows(afr_norm, amr_norm, eas_norm, eur_norm, sas_norm)

# Clean up chromosome factor levels
all_norm$CHR <- factor(all_norm$CHR, levels = c(1:22, "X"))

# Histogram of the number of variants per population 
ggplot(all_norm, aes(x = CHR, y = Variant_Count, fill = Population)) +
  geom_bar(stat = "identity") +
  facet_wrap(~Population, scales = "free_y") +
  ylab("Number of Variants") +
  xlab("Chromosome") +
  ggtitle("The Number of Variants Phenotype-Associated Population Variants Per Chromosome") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), plot.title = element_text(hjust = 0.5))

# Histogram of the normalised counts per chromosome for each population
ggplot(all_norm, aes(x = CHR, y = (Variants_per_bp*1e4), fill = Population)) +
  geom_bar(stat = "identity") +
  facet_wrap(~Population, scales = "free_y") +
  ylab(bquote("Normalized Variant Density" (x10^-4))) +
  xlab("Chromosome") +
  ggtitle("The Number of Variants Phenotype-Associated Population Variants Per Chromosome Adjusted for the Size of the Chromosome") +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), legend.position = "none", plot.title = element_text(hjust = 0.5))
#ggsave("figures/normalised_variants_phenotypes_per_chr.png", width = 12, height = 7)


```

### GWAS effect sizes

```{r effect_size}
#AFR_pheno <- read.table("gwas_1000_genomes/AFR_all_gwas.bed", header = F, sep = "\t")
#AMR_pheno <- read.table("gwas_1000_genomes/AMR_all_gwas.bed", header = F, sep = "\t")
#EAS_pheno <- read.table("gwas_1000_genomes/EAS_all_gwas.bed", header = F, sep = "\t")
#EUR_pheno <- read.table("gwas_1000_genomes/EUR_all_gwas.bed", header = F, sep = "\t")
#SAS_pheno <- read.table("gwas_1000_genomes/SAS_all_gwas.bed", header = F, sep = "\t")

colnames(AFR_pheno) <- colnames(AMR_pheno) <- colnames(EAS_pheno) <- colnames(EUR_pheno) <- colnames(SAS_pheno) <- c("CHR", "start_pos", "end_pos", "RSID", "Phenotype","risk_AF", "p_value", "OR", "PMID", "v_CHR","v_start_pos", "v_end_pos", "v_rsid", "REF", "ALT", "AF", "overlap")

AFR_pheno$CHR <- factor(AFR_pheno$CHR, levels = c(1:22, "X"))
AMR_pheno$CHR <- factor(AMR_pheno$CHR, levels = c(1:22, "X"))
EAS_pheno$CHR <- factor(EAS_pheno$CHR, levels = c(1:22, "X"))
EUR_pheno$CHR <- factor(EUR_pheno$CHR, levels = c(1:22, "X"))
SAS_pheno$CHR <- factor(SAS_pheno$CHR, levels = c(1:22, "X"))


ggplot(AFR_pheno, aes(x = CHR, mean(as.numeric(OR), na.rm=T))) + 
  geom_bar(stat = "identity", fill = "#F8766D") +
  labs(x = "Chromosome", y = "Average Effect Size", title = "The Average Effect Size of Phenotype-Associated Population Variants Per Chromosome in AFR") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme_classic() 

ggplot(AMR_pheno, aes(x = CHR, mean(as.numeric(OR), na.rm=T))) + 
  geom_bar(stat = "identity", fill = "#A3A500") +
  labs(x = "Chromosome", y = "Average Effect Size", title = "The Average Effect Size of Phenotype-Associated Population Variants Per Chromosome in AMR") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme_classic() 

ggplot(EAS_pheno, aes(x = CHR, mean(as.numeric(OR), na.rm=T))) + 
  geom_bar(stat = "identity", fill = "#00BF7D") +
  labs(x = "Chromosome", y = "Average Effect Size", title = "The Average Effect Size of Phenotype-Associated Population Variants Per Chromosome in EAS") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme_classic() 

ggplot(EUR_pheno, aes(x = CHR, mean(as.numeric(OR), na.rm=T))) + 
  geom_bar(stat = "identity", fill = "#00B0F6") +
  labs(x = "Chromosome", y = "Average Effect Size", title = "The Average Effect Size of Phenotype-Associated Population Variants Per Chromosome in EUR") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme_classic() 

ggplot(SAS_pheno, aes(x = CHR, mean(as.numeric(OR), na.rm=T))) + 
  geom_bar(stat = "identity", fill = "#E76BF3") +
  labs(x = "Chromosome", y = "Average Effect Size", title = "The Average Effect Size of Phenotype-Associated Population Variants Per Chromosome in SAS") +
  theme(plot.title = element_text(hjust = 0.5)) +
  theme_classic() 

```

## GWAS Ancestry Data

```{r ancestry}

set_colors <- c(
  AFR = "#F8766D",
  AMR = "#A3A500",
  EAS = "#00BF7D",
  EUR = "#00B0F6",
  SAS = "#E76BF3"
)

study_nos <- read.table("NHGRI_EBI_GWAS/NHGRI-EBI_ancestry.txt", header = T, sep = "\t")
colnames(study_nos) <- c("Superpopulation", "Number_of_Studies", "Initial", "Replication")
study_overlap <- read.table("gwas_1000_genomes/gwas_ancestry_phenotype_overlap.tsv", header = T, sep = "\t")
ancestry_info <- merge(study_nos, study_overlap, by = "Superpopulation")

study_plot <- ggplot(ancestry_info, aes(x = Superpopulation, y = Number_of_Studies, fill = Superpopulation)) +
  geom_col() +
  ylab("Study Count") +
  theme_classic() +
  theme(legend.position = "none", axis.title.x=element_blank()) +
  ggtitle("Number of Studies Performed in Each Population") + 
  theme(plot.title = element_text(hjust = 0.5))
study_plot

## Count how many of the studies were performed in the initial or replication stage of the study
ancestry_info <- ancestry_info %>%
  mutate(Percent_Not_Studied = Variants_Not_Studied / Total_Variants * 100) %>% mutate(Adjusted_Percent = Percent_Not_Studied / Number_of_Studies)
eur_study_count <- ancestry_info$Number_of_Studies[ancestry_info$Superpopulation == "EUR"]
ancestry_info <- ancestry_info %>%
  mutate(Normalised_Percent = Percent_Not_Studied * (eur_study_count / Number_of_Studies))


ancestry_plot <- ggplot(ancestry_info, aes(x = Superpopulation, y = Adjusted_Percent, fill = Superpopulation)) +
  geom_col() +
  ylab("Not Studied / Study Count (%)") +
  ggtitle("Proportion of the Phenotype-Associated Variants That Are Present in Populations Not Included in the Original GWAS Study") +
  theme_classic() +
  theme(legend.position = "none", axis.title.x=element_blank(), plot.title = element_text(hjust = 0.5))
ancestry_plot

combined_plot <- (study_plot + ancestry_plot) + 
  plot_layout(guides = "collect")
print(combined_plot)
#ggsave("figures/variant_ancestry_summary.png", combined_plot, width = 12, height = 5, dpi = 320)


```

### Looks at how many studies were performed in the initial or replication stages of the study per population
```{r stages}

study_nos <- read.table("NHGRI_EBI_GWAS/NHGRI-EBI_ancestry.txt", header = T, sep = "\t")
colnames(study_nos) <- c("Superpopulation", "Number_of_Studies", "Initial", "Replication")

df_long <- study_nos %>%
  pivot_longer(
    cols = c(Initial, Replication),
    names_to = "Stage",
    values_to = "Count"
  ) %>%
  mutate(
    Stage = factor(Stage, levels = c("Initial", "Replication"))
  )

set_colors <- c(
  AFR = "#F8766D",
  AMR = "#A3A500",
  EAS = "#00BF7D",
  EUR = "#00B0F6",
  SAS = "#E76BF3"
)

ggplot(
  df_long,
  aes(x = Superpopulation,
    y = Count,
    fill = Superpopulation,
    pattern = Stage,
    alpha = Stage)) +
  geom_col_pattern(
    position = position_stack(reverse = TRUE),
    colour = "black",
    linewidth = 0.2,
    # make the pattern more subtle
    pattern_fill = "black",
    pattern_colour = "black",
    pattern_density = 0.03,
    pattern_spacing = 0.03,
    pattern_size = 0.2,
    pattern_alpha = 0.25) +
  scale_fill_manual(values = set_colors) +
  scale_pattern_manual(values = c(
    Initial = "none",
    Replication = "stripe")) +
  scale_alpha_manual(values = c(
    Initial = 1,
    Replication = 0.75)) +
  labs(x = "Superpopulation",
    y = "Number of Studies",
    fill = "Superpopulation",
    pattern = "Study Stage",
    alpha = "Study Stage",
    title = "Number of GWAS Studies Performed in Each Superpopulation by Stage") +
  theme_minimal(base_size = 16) +
  theme(panel.grid.minor = element_blank(), plot.title = element_text(hjust = 0.5) )

``` 
### How many populations were involved in a GWAS 

```{r study_counts}

pop_study_counts <- read.table("NHGRI_EBI_GWAS/study_population_counts.txt", header = T)

ggplot(pop_study_counts, aes(x = as.factor(number_of_populations_studied), y = count)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  xlab("Number of Populations") +
  ylab("Number of Studies") +
  ggtitle("A Bar Plot of How Many Populations Involved in a GWAS") +
  theme_classic() +
  theme(plot.title = element_text(hjust = 0.5))

```

### Upset plot of the combinations of different populations involved in a GWAS 

```{r ancestry_combos}

gwas <- read_tsv("NHGRI_EBI_GWAS/GWAS_ancestry_with_unique_populations.tsv", col_names = TRUE)

# Keep only the columns of interest
gwas_long <- gwas %>%
  transmute(
    Study = .[[3]],
    Superpopulation = .[[14]]
  ) %>%
  filter(!is.na(Superpopulation), Superpopulation != "") %>%
  distinct()

# Keep only expected superpopulations
superpops <- c("AFR", "AMR", "EAS", "EUR", "SAS")

gwas_long <- gwas_long %>%
  filter(Superpopulation %in% superpops)

# Make presence/absence matrix
pop_presence_df <- gwas_long %>%
  mutate(value = 1) %>%
  pivot_wider(
    names_from = Superpopulation,
    values_from = value,
    values_fill = 0
  ) %>%
  as.data.frame()

# Remove Study column from matrix, but keep rownames
rownames(pop_presence_df) <- pop_presence_df$Study
pop_presence_df$Study <- NULL

# Make sure all superpopulation columns exist, even if one is absent
for (pop in superpops) {
  if (!pop %in% colnames(pop_presence_df)) {
    pop_presence_df[[pop]] <- 0
  }
}

# Reorder columns
pop_presence_df <- pop_presence_df[, superpops]

# Calculate set sizes and sort
set_sizes <- colSums(pop_presence_df)
sorted_sets <- names(sort(set_sizes, decreasing = TRUE))

# Plot
UpSetR::upset(
  pop_presence_df,
  nsets = length(sorted_sets),
  sets = sorted_sets,
  order.by = "freq",
  sets.bar.color = set_colors[sorted_sets],
  main.bar.color = "#333333",
  matrix.color = "black",
  matrix.dot.alpha = 1,
  set_size.show = FALSE,
  text.scale = c(1.7, 1.6, 1.4, 0, 1.7, 0),
  keep.order = TRUE
)

```

### Looks at the number of fixed variants (AF=1) that were present in a population where the study wasn't performed

```{r fixed_ancestry}

fixed_af_ancestry <- read.table("preliminary_exploration/gwas_unstudied_fixed_AF.txt", sep = "\t", header = T)

ggplot(fixed_af_ancestry, aes(x = Superpopulation, y = AF_1_Not_Studied_Count, fill = Superpopulation)) +
  geom_col() +
  labs(title = "Number of Fixed Variants Present in Populations That Were Not Studied",
    y = "Number of Fixed Variants") +
  theme_classic() +
  theme(legend.position = "none") +
  theme(plot.title = element_text(hjust = 0.5), axis.title.x=element_blank())


```

## GENCODE data

```{r gencode}

regions <- read.table("preliminary_exploration/gencode/gencode_population_region_summary.txt", header = T, sep = "\t")
regions <- subset(regions, select = -c(gene))
gene_breakdown <- read.table("preliminary_exploration/gencode/gencode_population_gene_summary.txt", header = T, sep = "\t")
colnames(regions)[1] <- colnames(gene_breakdown)[1] <- "Population"

regions_long <- regions %>%
  pivot_longer(cols = -Population, names_to = "Feature", values_to = "Count") %>% mutate(Count_scaled = Count / 1e6)
feature_order <- c("transcript", "exon", "CDS", "UTR", "start_codon", "stop_codon")
# Set the order of the variables
regions_long$Feature <- factor(regions_long$Feature, levels = feature_order)

# Region Plots
ggplot(regions_long, aes(x = Feature, y = Count_scaled, fill = Population)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  theme_classic() +
  xlab("Feature Type") +
  ylab(bquote("Count "(x10^6))) +
  ggtitle("Number of Variants that Occured in Each GENCODE Feature Type by Population")+
  theme(legend.position = "none", plot.title = element_text(hjust = 0.5))
#ggsave("figures/gene_features_gencode.png", dpi=300, width = 6, height = 4)

# regions log transformed 
ggplot(regions_long, aes(x = Feature, y = Count, fill = Population)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  scale_y_continuous(
    trans = "log10",
    breaks = 10^(0:7),
    labels = trans_format("log10", math_format(10^.x)),
    expand = c(0, 0),   # removes gap between axis and data
    limits = c(1, NA),  # forces y-axis to start at 10^0 without cutting bars
    minor_breaks = rep(1:9, 7) * 10^rep(0:6, each = 9)
  ) +
  theme_classic() +
  xlab("Feature Type") +
  ylab(expression("Count (Log" [10] * ")")) + 
  ggtitle("Log-Transformed Number of Variants that Occured in Each GENCODE Feature Type by Population") +
  theme(legend.position = "none", plot.title = element_text(hjust = 0.5))
#ggsave("figures/population_gene_feature_breakdown.png", dpi=300, width = 6, height = 4)

#remove pseudogenes
genes <- gene_breakdown[,!grepl("_pseudogene$",names(gene_breakdown))]
genes_long <- genes %>%
  pivot_longer(cols = -Population, names_to = "Gene_Type", values_to = "Count")  %>%
  mutate(Count_scaled = Count / 1e5)

biotype_map <- list(
  "Protein-coding" = c("protein_coding"),
  "lncRNA" = c("lncRNA", "TEC"),
  "Pseudogene" = c("processed_pseudogene", "unprocessed_pseudogene", "transcribed_unprocessed_pseudogene",
                   "transcribed_processed_pseudogene", "unitary_pseudogene", "transcribed_unitary_pseudogene"),
  "Immune (IG/TR)" = c("IG_C_gene", "IG_V_gene", "IG_J_gene", "IG_D_gene",
                       "TR_C_gene", "TR_V_gene", "TR_J_gene", "TR_D_gene", 
                       "IG_C_pseudogene", "IG_V_pseudogene"),
  "ncRNA" = c("miRNA", "snRNA", "snoRNA", "misc_RNA", "rRNA", "scaRNA", "vault_RNA", "rRNA_pseudogene")
)

biotype_lookup <- unlist(lapply(names(biotype_map), function(group) {
  setNames(rep(group, length(biotype_map[[group]])), biotype_map[[group]])
}))

# Assign groups
gene_counts <- genes_long %>%
  mutate(Biotype_Group = biotype_lookup[Gene_Type]) %>%
  filter(!is.na(Biotype_Group))  # remove unmapped types if needed

grouped_counts <- gene_counts %>%
  group_by(Population, Biotype_Group) %>%
  summarise(Total = sum(Count), .groups = "drop")

biotype_order <- grouped_counts %>%
  group_by(Biotype_Group) %>%
  summarise(Sum = sum(Total)) %>%
  arrange(desc(Sum)) %>%
  pull(Biotype_Group)

# Set factor levels to order the x-axis
grouped_counts$Biotype_Group <- factor(grouped_counts$Biotype_Group, levels = biotype_order)

ggplot(grouped_counts, aes(x = Biotype_Group, y = Total, fill = Population)) +
  geom_bar(stat = "identity", position = "dodge") +
    scale_y_continuous(
    trans = "log10",
    breaks = 10^(0:7),
    labels = trans_format("log10", math_format(10^.x)),
    expand = c(0, 0),   # removes gap between axis and data
    limits = c(1, NA),  # forces y-axis to start at 10^0 without cutting bars
    minor_breaks = rep(1:9, 7) * 10^rep(0:6, each = 9)
  ) +
  theme_classic() +
  xlab("Biotype Group") +
  ylab(expression("Count (Log" [10] * ")")) + 
  ggtitle("Log-Transformed Number of Variants that Occured in Each GENCODE Biotype Group by Population") +
  theme_classic() +
    theme(
    axis.text.x = element_text(angle = 45, hjust = 1),
    text = element_text(size = 14), 
    plot.title = element_text(hjust = 0.5))
#ggsave("figures/population_gene_biotypes_grouped.png", dpi=300, width = 6, height = 4)

# Order gene types by descending total count and select top 10§
top_genes <- genes_long %>%
  group_by(Gene_Type) %>%
  summarise(total_count = sum(Count)) %>%
  arrange(desc(total_count)) %>%
  slice_head(n = 10)

df_top10 <- genes_long %>%
  filter(Gene_Type %in% top_genes$Gene_Type) %>%
  mutate(Gene_Type = factor(Gene_Type, levels = top_genes$Gene_Type))

ggplot(df_top10, aes(x = Gene_Type, y = Count, fill = Population)) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_y_continuous(trans = "log10",
                     breaks = trans_breaks("log10", function(x) 10^x),
                     labels = trans_format("log10", math_format(10^.x))) +
  theme_classic()+
  theme(axis.text.x = element_text(angle = 45, hjust = 1),
        text = element_text(size = 14)) +
  labs(y = "Count (log10)",
    x = "Gene Type",
    title = "Top 10 Gene Types by Presence of Phenotype-Associated Population Variants, Log-Transformed")
#ggsave("figures/gene_types_gencode.png", dpi=300, width = 6, height = 4)

ggplot(df_top10, aes(x = Gene_Type, y = Count, fill = Population)) +
  geom_bar(stat = "identity", position = position_dodge()) +
  theme_classic() +
  xlab("Gene Type") +
  ylab(bquote("Count "(x10^6))) +
  ggtitle("Top 10 Gene Types by Presence of Phenotype-Associated Population Variants, Log-Transformed") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1), plot.title = element_text(hjust = 0.5))

```

## ENCODE cCRE data

```{r ccres, echo=FALSE}

# read in files for the total number of variants for each superpopulation
ccre <- read.table("preliminary_exploration/cCRE/ccre_counts.txt", sep = "\t", header = T)
ccre$adjusted_total <- ccre$total_ccres /1e4


# plotting total variants per population
prom <- ggplot(ccre, aes(Population, promoter, fill = Population)) + geom_bar(stat="identity") +
  ylab("Number of Variants ") +
  xlab("Population") +
  ggtitle("Number of Phenotype-Associated Population Variants in Regions with Promoter-Like Signals in the ENCODE cCRE Database") + 
  theme_classic() +
  theme(legend.position="none", plot.title = element_text(hjust = 0.5))
prom
# Save high-resolution image
#ggsave("figures/population_promoter_count.png", prom, dpi = 300, width = 6, height = 4)

enh <- ggplot(ccre, aes(Population, enhancer, fill = Population)) + geom_bar(stat="identity") +
  ylab("Number of Variants ") +
  xlab("Population") +
  ggtitle("Number of Phenotype-Associated Population Variants in Regions with Enhancer-Like Signals in the ENCODE cCRE Database") + 
  theme_classic() +
  theme(legend.position="none", plot.title = element_text(hjust = 0.5))
enh
#ggsave("figures/population_enhancer_count.png", enh, dpi = 300, width = 6, height = 4)

total <- ggplot(ccre, aes(Population, adjusted_total, fill = Population)) + geom_bar(stat="identity") +
  ylab(bquote("Number of Variants "(x10^4))) +
  xlab("Population") +
  ggtitle("Number of Phenotype-Associated Population Variants in Suspected CRE Regions in the ENCODE cCRE Database") + 
  theme_classic() +
  theme(legend.position="none", plot.title = element_text(hjust = 0.5))
total
#ggsave("figures/ccre_count.png", total, dpi = 300, width = 6, height = 4)


```

## GTEx eQTL data

```{r eqtls, echo=FALSE}

# read in files for the total number of variants for each superpopulation
eqtls <- read.table("preliminary_exploration/GTEx_eQTLs/eQTL_counts.txt", sep = "\t", header = F)
colnames(eqtls) <- c("Population", "eQTLs")
eqtls$adjusted_total <- eqtls$eQTLs /1e5

# plotting total variants per population
eqtl_plot <- ggplot(eqtls, aes(Population, adjusted_total, fill = Population)) + geom_bar(stat="identity") +
  ylab(bquote("Number of Variants "(x10^5)))  +
  theme_classic() +
  ggtitle("Number of Phenotype-Associated Population Variants in eQTL regions from the GTEx eQTL Database") +
  theme(legend.position="none",
        axis.title.x = element_blank(),
        plot.title = element_text(hjust = 0.5)) 
eqtl_plot
#ggsave("figures/population_eQTL_count.png", eqtl_plot, dpi = 300, width = 6, height = 4)

```
